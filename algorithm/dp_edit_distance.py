...
class DPEditDistance:
    ...
    """
    def sort_smallest(self, L, w):  # O(NX), X is the complexity of the mapping
        if not L:
            return

        word = L[0]
        distance = self.least_edit_distance(word, w)

        for i in range(1, len(L)):
            d = self.least_edit_distance(L[i], w)
            if d < distance:
                distance = d
                word = L[i]
    ...
    """
    def sort_largest(self, L, w):  # O(NX), X is the complexity of the mapping
        if not L:
            return
    ...
    def sort_largest_k(self, L, w, k):  # O(NX + NlogN)
        if not L:
            return

        heap = []
        for l in L:
            d = self.least_edit_distance(l, w)
            if len(heap) < k:
    ...
    def sort_smallest_k(self, L, w, k):  # O(NX + NlogN + k)
        if not L:
            return
    ...
    """
    def sort_tops(self, L, w):  # O(NX + NlogN + k)
        res = {}
        for l in L:
            res[self.least_edit_distance(l, w)] = l  # O(NX)

        top5 = sorted(res.items(), key=lambda item: item[0])[:5]  # Time Complexity O(NlogN + 5)

        return top5

    def heap_tops(self, L, w):  # O(NX + NlogN + klogN)
        res = []
        for l in L:
            heapq.heappush(res, (self.least_edit_distance(l, w), l))  # O(N * (X + logN))
            # if len(res) > 5:
            #     heapq.heappop(res)
    ...
    """
    def least_edit_distance_bf(self, word1, word2):
        def dp(i, j):
            if i == 0:
                return j
            if j == 0:
    ...
    @return: integer the edit distance
    """
    def least_edit_distance(self, word1, word2):
...
if __name__ == '__main__':
    ed = DPEditDistance()
    print(ed.least_edit_distance("horse", "ros"))  # expect 3
    print(ed.least_edit_distance_bf("intention", "execution"))  # expect 5
    print(ed.least_edit_distance("aDb", "adb"))  # expect 1
    print(ed.least_edit_distance("trinitrophenylmethylnitramine", "dinitrophenylhydrazine"))  # expect 10
    print(ed.sort_tops(["aaaa", "aa", "a", "aaa", "aaaaa", "aaaaaaa"], "a"))
    print(ed.heap_tops(["aaaa", "aa", "a", "aaa", "aaaaa", "aaaaaaa"], "a"))
    print(ed.sort_smallest(["aaaa", "aa", "a", "aaa", "aaaaa", "aaaaaaa"], "a"))  # expected (0, 'a')
    print(ed.sort_largest(["aaaa", "aa", "a", "aaa", "aaaaa", "aaaaaaa"], "a"))  # expected (6, 'aaaaaaa')
    print(ed.sort_largest_k(["aaaa", "aa", "a", "aaa", "aaaaa", "aaaaaaa"], "a", 5))
    print(ed.sort_smallest_k(["aaaa", "aa", "a", "aaa", "aaaaa", "aaaaaaa"], "a", 5))
