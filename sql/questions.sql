-- display employee name and his/her manager name
-- table employee
-- employee_id, ename, mgr_id
...
select deptno, sal, max(rank)
from base
group by 1, 2;

select country,
SUM(case when medal == 'Gold' then 1 else 0) as Gold,
SUM(case when medal == 'Silver' then 1 else 0) as Silver,
SUM(case when medal == 'Bronze' then 1 else 0) as Bronze
from olympic
group by 1
